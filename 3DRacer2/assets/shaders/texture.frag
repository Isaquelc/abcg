...
//Blinn-Phong for no texture
vec4 BlinnPhong_standard(vec3 N, vec3 L, vec3 V) {
  N = normalize(N);
  L = normalize(L);
  ...
  return ambientColor + diffuseColor + specularColor;
  // return diffuseColor; //vec4((N + 1.0) / 2.0, 1.0); 
}
...
void main() {
  ...
  if (mappingMode == 4) {
    color = BlinnPhong_standard(fragN, fragL, fragV);
  }
